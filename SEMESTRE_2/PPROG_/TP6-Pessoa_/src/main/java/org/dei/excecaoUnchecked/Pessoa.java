...
public class Pessoa implements Comparable<Pessoa> {
    ...
    public void setNome(String nome) {
        if (nome == null || nome.trim().isEmpty()) {
            throw new ArgumentoForaDosLimitesException("Nome é um campo obrigatório logo não pode estar vazio!!");
        }
        this.nome = nome;
    }
    ...
    public void setIDCivil(int idCivil) {
        if (idCivil <= 0 || idCivil > 99999999) {
            throw new ArgumentoForaDosLimitesException("ID Civil deve ser positivo com no máximo 8 dígitos!!");
        }
        this.idCivil = idCivil;
    }
